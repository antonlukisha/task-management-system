...
public class TaskDataService implements TaskDataServiceInterface {
    ...
    public Task getTaskFromCacheOrDatabase(Long taskId) {
        try {
            ...
            logger.error("Failed to connect with Redis Server for retrieving cache data. Check connection");
            throw TaskException.of(HttpStatus.INTERNAL_SERVER_ERROR, "Server error");
        }
    ...
    public void deleteTaskFromCacheOrDatabase(Long taskId) {
        try {
            ...
            logger.error("Failed to connect with Redis Server for deleting expired cache data. Check connection");
            throw TaskException.of(HttpStatus.INTERNAL_SERVER_ERROR, "Server error");
        }
    }
    ...
    public void updateTaskFromCacheOrDatabase(Long taskId, Task task) {
        ...
        logger.error("Failed to connect with Redis Server for updating cache data. Check connection");
            throw TaskException.of(HttpStatus.INTERNAL_SERVER_ERROR, "Server error");
        }
    }
}
